#!/usr/bin/env bash
# Installs CDD methodology as Codex skill in ~/.codex/skills
set -e

source_path="${BASH_SOURCE[0]}"
while [ -L "$source_path" ]; do
  source_dir="$(cd -P "$(dirname "$source_path")" >/dev/null 2>&1 && pwd)"
  source_path="$(readlink "$source_path")"
  case "$source_path" in
    /*) ;;
    *) source_path="$source_dir/$source_path" ;;
  esac
done
cd "$(cd -P "$(dirname "$source_path")" >/dev/null 2>&1 && pwd)"

# Create skills directory if it doesn't exist
mkdir -p ~/.codex/skills/concept_driven_design/references

# Generate and install the skill
./build > ~/.codex/skills/concept_driven_design/SKILL.md
./build-concepts-reference > ~/.codex/skills/concept_driven_design/references/concepts.md

echo "Installed concept-driven-design skill to ~/.codex/skills/concept_driven_design/SKILL.md"
