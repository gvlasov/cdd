...
complete -c cdd -n '__fish_use_subcommand; and begin; test -d ./commands/dev; or test -d ./platform/cli; end' -a '(set seen help init print self-upgrade; for dir in ./commands/dev ./platform/cli; test -d $dir; or continue; for filepath in $dir/*; test -f $filepath; or continue; set name (basename $filepath); contains -- $name $seen; and continue; set --append seen $name; echo $name; end; end)' -d 'Run project command'
