...
complete -c cdd -n '__fish_use_subcommand; and test -d ./platform/cli' -a '(for filepath in ./platform/cli/*; test -f $filepath; or continue; set name (basename $filepath); contains -- $name help init self-upgrade; or echo $name; end)' -d 'Run project command'
