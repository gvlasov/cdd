...
complete -c cdd -n '__fish_use_subcommand; and test -d ./commands' -a '(set seen help init print self-upgrade; for filepath in ./commands/*; test -f $filepath; or continue; set name (basename $filepath); contains -- $name $seen; and continue; set --append seen $name; echo $name; end)' -d 'Run project command'
